...
class VisVersions:
    ...
    @staticmethod
    def try_parse(version_str) -> tuple[bool, VisVersion | None]:
        if version_str == "3-4a":
            return True, VisVersion.v3_4a
        if version_str == "3-8a":
            return True, VisVersion.v3_8a
        if version_str == "3-6a":
            return True, VisVersion.v3_6a
        if version_str == "3-5a":
            return True, VisVersion.v3_5a
        if version_str == "3-7a":
            return True, VisVersion.v3_7a
        return False, None

    @staticmethod
    def parse(version_str: str) -> VisVersion:
        success, version = VisVersions.try_parse(version_str)
        if not success:
