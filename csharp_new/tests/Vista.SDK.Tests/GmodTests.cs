...
public class GmodTests
{
    public static IEnumerable<object[]> Test_Vis_Versions =>
        Vista.SDK.VisVersions.All.Select(x => new object[] { x }).ToArray();
    ...
    [Fact]
    public void Test_Gmod_Traversal()
    {
        var (_, vis) = VISTests.GetVis();
        var gmod = vis.GetGmod(VisVersion.V37a);

        // var finished = gmod.Traverse(
        //     (parents, node) =>
        //     {
    ...
    //     var gmod = vis.GetGmod(VisVersion.V34a);
